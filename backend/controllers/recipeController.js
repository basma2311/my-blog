...
export const getOneRecipe = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        message:
          "Une erreur s'est produite lors de la tentative de récupération la recette.",
      });
  }
};
...
// récupérer les recettes healthy
...
//récupérer les recettes de dessert healthy
export const getHealthyDessertRecipes = async (req, res) => {
  try {
    const healthyDessertRecipes = await Recipe.find({ category: "Dessert", healthy: true });

    if (!healthyDessertRecipes) {
      return res.status(404).json({ message: "Aucune recette de dessert healthy trouvée!" });
    }

    res.status(200).json(healthyDessertRecipes);
  } catch (err) {
    res.status(500).json({
      message: "Une erreur s'est produite lors de la récupération des recettes de dessert healthy.",
    });
  }
};

//récupérer les plat et les entrée healthy

export const getHealthyStarterAndDishRecipes = async (req, res) => {
  try {
    const healthyPlatsAndEntrees = await Recipe.find({ $or: [{ category: "Plat" }, { category: "Entrée" }], healthy: true });

    if (!healthyPlatsAndEntrees) {
      return res.status(404).json({ message: "Aucune recette healthy trouvée pour les plats ou les entrées!" });
    }

    res.status(200).json(healthyPlatsAndEntrees);
  } catch (err) {
    res.status(500).json({
      message: "Une erreur s'est produite lors de la récupération des recettes healthy pour les plats ou les entrées.",
    });
  }
};
